...
### 난이도 표기 (`lib/difficulty.ts`)

OPIc 난이도는 하나의 숫자가 아니라 **두 번의 선택이 만드는 조합**입니다.

```
initial   자가평가 1~6단계          -> 문제 세트와 문항 수를 결정
adjusted  7번 문항 후 1회 조정        -> 8번 이후 표현 수준만 한 칸 이동
label     `${initial}-${adjusted}`   -> 3-3, 4-4, 5-5, 5-6 ...
```

성립하는 조합 16가지를 학습 화면에서 직접 골라 연습할 수 있습니다.
